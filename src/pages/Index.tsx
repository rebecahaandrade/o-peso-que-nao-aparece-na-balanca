...
import { Check, X, Heart, Eye, Brain, Flame, Shield, BookOpen, Feather, Star, Quote, Sparkles, Target, Lightbulb } from "lucide-react";
...
const PlaceholderImage = ({ aspect = "16/9", label }: { aspect?: string; label?: string }) => (
  <div
    className={`relative w-full rounded-2xl overflow-hidden bg-gradient-to-br from-primary/10 via-secondary to-rose-soft border border-border/30 shadow-md`}
    style={{ aspectRatio: aspect }}
  >
    <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-muted-foreground/60">
      <BookOpen className="w-10 h-10" />
      {label && <span className="text-xs font-body tracking-wide uppercase">{label}</span>}
    </div>
  </div>
);
  ...
  <section className="min-h-[90vh] flex items-center justify-center px-6 md:px-8 py-20 bg-gradient-to-b from-rose-soft to-background">
    <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-12 items-center">
      ...
      <div className="order-first md:order-last">
        <PlaceholderImage aspect="3/4" label="Capa do ebook" />
      </div>
    </div>
  </section>
);
  ...
  <SectionWrapper>
    <div className="space-y-8">
      <div className="grid sm:grid-cols-3 gap-4">
        {[
          { icon: Eye, text: "Se você já tentou emagrecer mais vezes do que consegue contar…" },
          { icon: Heart, text: "Se já prometeu \"agora vai\" olhando no espelho…" },
          { icon: Shield, text: "Se já sentiu vergonha do próprio corpo, mesmo tentando ser forte…" },
        ].map(({ icon: Icon, text }) => (
          <Card key={text} className="bg-card/80 border-border/40 hover:shadow-lg transition-shadow duration-300">
            <CardContent className="p-6 text-center space-y-4">
              <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center mx-auto">
                <Icon className="w-6 h-6 text-primary" />
              </div>
              <p className="font-body text-base text-foreground/80 italic leading-relaxed">{text}</p>
            </CardContent>
          </Card>
        ))}
      </div>
  ...
  <SectionWrapper className="bg-warm-dark text-primary-foreground">
    <div className="grid md:grid-cols-5 gap-10 items-center">
      ...
      <div className="md:col-span-2">
        <PlaceholderImage aspect="4/5" label="Imagem emocional" />
      </div>
    </div>
  </SectionWrapper>
);
  ...
  <SectionWrapper className="bg-rose-soft">
    <div className="space-y-8">
      ...
      <div className="grid md:grid-cols-2 gap-8 items-center">
        <PlaceholderImage aspect="1/1" label="Jornada pessoal" />
        <div className="space-y-4">
          <Card className="bg-card/60 border-none shadow-md">
            <CardContent className="p-6">
              <Quote className="w-6 h-6 text-primary mb-3" />
              <p className="font-display text-2xl italic text-foreground">"Eu sempre fui a gordinha."</p>
            </CardContent>
          </Card>
          <div className="font-body text-lg leading-relaxed space-y-3 text-foreground/85">
            <p>Não a forte. Não a confiante.<br />A engraçada. A simpática.</p>
        ...
        {[
          ...
          <Card key={text} className="bg-primary/10 border-primary/20">
            <CardContent className="p-5 text-center">
              <p className="font-display text-lg md:text-xl font-semibold text-primary">👉 {text}</p>
            </CardContent>
          </Card>
        ))}
      </div>
    ...
    <div className="space-y-8">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {identificationItems.map(({ icon: Icon, text }) => (
          <Card key={text} className="bg-card border-border/40 hover:border-primary/30 transition-colors duration-300">
    ...
    <div className="space-y-8 text-center">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2">
        {bookTopics.map(({ icon: Icon, label }) => (
          <Card key={label} className="bg-card/70 border-border/30 hover:shadow-md transition-shadow duration-300">
            <CardContent className="p-5 flex flex-col items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center">
                <Icon className="w-6 h-6 text-primary" />
              </div>
              <span className="font-body text-sm md:text-base text-foreground/80 font-medium">{label}</span>
            </CardContent>
          </Card>
        ))}
      </div>
...
const BenefitsSection = () => (
  <SectionWrapper>
    <div className="grid md:grid-cols-2 gap-10 items-center">
      <div className="space-y-8">
        <h2 className="font-display text-3xl md:text-4xl font-semibold">O que você vai encontrar neste livro</h2>
      ...
      <PlaceholderImage aspect="4/5" label="Páginas do ebook" />
    </div>
  </SectionWrapper>
);
...
const PriceSection = () => (
  <SectionWrapper className="bg-warm-dark text-primary-foreground">
    <div className="grid md:grid-cols-2 gap-10 items-center">
      <PlaceholderImage aspect="1/1" label="Depoimento / Resultado" />
      <div className="space-y-8 text-center md:text-left">
